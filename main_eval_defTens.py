...
from util_code.basic_util import to_matrix, to_matrix_extended, gradient, divergence
...
## Training parameters
logf_network_file = "data/state_log_f.pt"    # state-file for the previously trained single-point pdf network (see nf_optim files)
G_network_file    = "data/state_epoch_10.pt"  # state-file from previous training to use for initialization of the MULTI-TIME stat. network
                    # (set to None for random initializition)
out_dir = "eval_results_deform_tens/" # directory to save results in
...
initialization_method = "Lyapunov" # method to use for initializing the simulation available options: 
...
class Evaluator():
    ...
    def evaluate(self, task):
        ...
        D_ens = to.ones((A_ens.shape[0],A_ens.shape[1],self.N_var,self.N_var),device=self.device)
        ...
        for write_it in range(0 if self.restart_sim_it == None else self.restart_sim_it,\
            ...
            for it in range(self.N_steps-1):
                with to.no_grad():
                    ...
                    D  = D_ens[it].to(self.device)
                N0 = self.rhs(A)
                M0 = self.rhs_D(A,D,N0)
                with to.no_grad():
                    A1 = A + self.dt*N0
                    D1 = D + self.dt*M0
                N1 = self.rhs(A1)
                M1 = self.rhs_D(A1,D1,N1)

                with to.no_grad():
                    A_ens[it+1] = A + .5*self.dt*(N0 + N1)
                    D_ens[it+1] = D + .5*self.dt*(M0 + M1)
                    ...
                    D_ens[it+1] = to.where(D_ens[it+1]==D_ens[it+1], D_ens[it+1], to.zeros(D.shape, device=D.device, requires_grad=False))
                    D_ens[it+1] = to.where(to.abs(D_ens[it+1])<self.divergence_threshold, D_ens[it+1], to.zeros(D.shape, device=D.device, requires_grad=False))
                    #
                    if self.rank==0:
                        if div_count>.5:
            ...
            # write out deformation tensor
            D_ens_full=to.empty((D_ens.shape[0],D_ens.shape[1],9,9),device=self.device)
            D_ens_full[:,:,:8,:8]=1*D_ens
            D_ens_full[:,:,:8,8] = - D_ens[:,:,:,0] - D_ens[:,:,:,4]
            D_ens_full[:,:,8,:] = - D_ens_full[:,:,0,:] - D_ens_full[:,:,4,:]
            D_ens_full=D_ens_full.reshape(D_ens_full.shape[0],D_ens_full.shape[1],3,3,3,3)
            if self.rank==0:
                ###Init h5 file
                with h5py.File(self.out_dir+"/D_ens_{}".format(self.N_part)+".h5", 'a') as f:
                    dset = "D_ens_{}".format(write_it)
                    f.create_dataset(dset, shape=(self.N_steps//4, self.world_size*self.N_part//stride, 3, 3, 3, 3))
                    f[dset][:,0*self.N_part//stride:(0+1)*self.N_part//stride] = D_ens_full.detach().cpu()
                    for i in range(1,self.world_size):
                        dist.recv(D_ens_full.contiguous(), i, tag=i)
                        f[dset][:,i*self.N_part//stride:(i+1)*self.N_part//stride] = D_ens_full.detach().cpu()
            else:
                dist.send(D_ens_full.contiguous(), dst=0, tag=self.rank)
            dist.barrier()
    ...
    def rhs_D(self, A,D,N):
        dNdA=to.empty(A.shape[0],self.N_var,self.N_var,device=A.device)
        for i in range(self.N_var):
            dNdA[:,i]=gradient(N[:,i],A).detach()

        return to.einsum("sik,skp->sip",dNdA,D)
